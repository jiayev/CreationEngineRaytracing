...
namespace Pass
{
	...
	void RTDI::ResolutionChanged([[maybe_unused]] uint2 resolution)
	{
		m_DirtyBindings = true;
	}

	void RTDI::CreateRootSignature()
	{
	...
	bool RTDI::CreateRayTracingPipeline()
	{
		...
		// Compile Libraries
		auto rayGenLib = ShaderUtils::CompileShaderLibrary(device, L"data/shaders/raytracing/RTDI/RayGeneration.hlsl", defines);
		auto missLib = ShaderUtils::CompileShaderLibrary(device, L"data/shaders/raytracing/RTDI/Miss.hlsl", defines);
		auto hitLib = ShaderUtils::CompileShaderLibrary(device, L"data/shaders/raytracing/RTDI/ClosestHit.hlsl", defines);
	...
	bool RTDI::CreateComputePipeline()
	{
		...
		ShaderUtils::CompileShader(rayGenBlob, L"data/shaders/raytracing/RTDI/RayGeneration.hlsl", defines, L"cs_6_5");
	...
	void RTDI::Execute(nvrhi::ICommandList* commandList)
	{
		CheckBindings();
